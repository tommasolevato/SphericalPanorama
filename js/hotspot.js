...
function getHotspot() {
  ...
  for (var i = 0; i < hotspotArray.length; i++) {
    ...
    var position = new THREE.Vector3(
      parseInt(hotspot["xPosition"]),
      parseInt(hotspot["yPosition"]),
      parseInt(hotspot["zPosition"])
    );
...
function portal(html, hotspotPosition, width, heigth, leftOrRight, color) {
  ...
  var inside = 30;
  var frame = makeFrame(
    parseInt(width) + inside * 2,
    parseInt(heigth) + inside * 2,
    50,
    inside,
    45,
    color
  ...
  cssObject.position = new THREE.Vector3(
  ...
  frame.position = new THREE.Vector3(
    ...
    planeMesh.position.y,
    planeMesh.position.z - planeMesh.position.z / planeMesh.position.x
  );
  frame.rotation = new THREE.Vector3(
...
function makeHotspot(position, id) {
  ...
  var torusGeometry = new THREE.LatheGeometry(
    pts,
    12,
    0,
    (Math.PI * 170) / 360
  ...
  var materials3 = [
    new THREE.MeshLambertMaterial({
      map: THREE.ImageUtils.loadTexture(
        "images/pdf_icon_horizontal_reflected.png"
      )
    })
  ];
  ...
  torus3.rotation.z =
    torus3.rotation.z + (Math.PI * 3) / 4 + (Math.PI * 5) / 360;
  torus3.name = "PDF";
  torus3.hotspotId = id;
  markers.push(torus3);
  scene.add(torus3);
  ...
  torus4.rotation.z =
    torus4.rotation.z + (Math.PI * 5) / 4 + (Math.PI * 5) / 360;
  torus4.name = "Object";
  torus4.hotspotId = id;
  markers.push(torus4);
  scene.add(torus4);
  ...
  var sphere = new THREE.Mesh(
    new THREE.SphereGeometry(10, 80, 120),
  ...
  sphere.rotation.y =
    sphere.rotation.y +
    Math.PI / 2 +
    Math.atan(parseFloat(sphere.position.z) / parseFloat(sphere.position.x)); //FIXME
}
...
function manageHotspot() {
  ...
  function restoreHotspotPosition(hotspotId) {
    ...
    if (selectedFrame !== undefined)
      selectedFrame.position = new THREE.Vector3(
  ...
  function rotateToHotspotPosition() {
    ...
    var response = XYZtoLonLat(
      hotspotInfo["xPosition"],
      hotspotInfo["yPosition"],
      hotspotInfo["zPosition"]
    );
    smoothLonLatTransition(response[0], response[1], 3);
  }
  ...
  switch (interactiveObject.name) {
    case "Gallery":
      if (rightPosition) {
        ...
        portal(
          hotspotInfo["Source"] + "?id=" + hotspotInfo["IdName"],
    ...
    case "PDF":
      if (!rightPosition) {
        ...
        portal(
          hotspotInfo["Source"] + "?id=" + pdfSource,
          interactiveObject.position,
          hotspotInfo["Width"],
          hotspotInfo["Height"],
          "left",
...
function makeFrame(width, height, radius, inside, cornice, color) {
  ...
  for (var angle = -alpha; angle < alpha; angle += detail)
    californiaPts.push(
      new THREE.Vector2(
        -width / 2 - (radius - inside) + Math.cos(angle) * radius,
  ...
  californiaPts.push(
    new THREE.Vector2(
      center + 0.00001 + cornice - width / 2,
      +cornice - height / 2
    )
  );
  californiaPts.push(
    new THREE.Vector2(center - cornice + width / 2, +cornice - height / 2)
  );
  californiaPts.push(
    new THREE.Vector2(center - cornice + width / 2, -cornice + height / 2)
  );
